...
const ReportModal = ({ isOpen, onClose, post, userData }) => {
  ...
  return (
    ...
    >
      <div 
        className="bg-white px-6 pt-10 pb-5 md:p-8 rounded-2xl shadow-2xl w-full max-w-lg relative"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Tombol Tutup (Opsional, tapi praktik yang baik) */}
        <button 
          onClick={onClose} 
          className="absolute top-3 right-4 grid h-9 w-9 place-items-center rounded-full text-slate-500 hover:bg-slate-100 transition-colors"
        ...
        <div className="space-y-2 mb-6">
          ...
          <p className="text-slate-500">Pilih alasan Anda melaporkan konten ini.</p>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="space-y-3 mb-5 md:mb-8">
            {reasons.map(r => (
              <label 
                key={r} 
                className={`flex items-center px-4 py-2 md:p-4 rounded-xl border-2 transition-all cursor-pointer ${reason === r ? 'bg-primary/10 border-primary shadow-sm' : 'border-slate-200 hover:border-slate-300'}`}
              >
                <input 
                  type="radio" 
                  name="reason" 
